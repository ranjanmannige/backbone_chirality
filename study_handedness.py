...
if 1:
	# Just putzing around with Symbolic Python to see if it can simplify miyazawa's equations
	import sympy as sp
	k1 = sp.Symbol('k1', positive=True)
	k2 = sp.Symbol('k2', positive=True)
	k3 = sp.Symbol('k3', positive=True)
	k4 = sp.Symbol('k4', positive=True)
	m1 = sp.Symbol('m1', positive=True)
	m2 = sp.Symbol('m2', positive=True)
	m3 = sp.Symbol('m3', positive=True)
	...
	# values for point 1	
	phi1,psi1,theta1,d1 = sp.symbols('phi1,psi1,theta1,d1')
	theta1 =  2 * sp.acos( + k1 * sp.cos((+phi1+psi1+omega)/2)
	                      - k2 * sp.cos((+phi1-psi1+omega)/2)
	                      - k3 * sp.cos((+phi1+psi1-omega)/2)
	                      - k4 * sp.cos((-phi1+psi1+omega)/2)
	                    )
	d1 =  + m1*k1*sp.sin((+phi1+psi1+omega)/2) - m2*k2*sp.sin((+phi1-psi1+omega)/2) \
	     - m3*k3*sp.sin((+phi1+psi1-omega)/2) - m4*k4*sp.sin((-psi1+psi1+omega)/2)
	d1 = d1 / sp.sin(theta1/2)
	
	# values for point 2
	phi2,phi2,theta2,d2 = sp.symbols('phi2,phi2,theta2,d2')
	theta2 =  2 * sp.acos( + k1 * sp.cos((+phi2+phi2+omega)/2)
	                      - k2 * sp.cos((+phi2-phi2+omega)/2)
	                      - k3 * sp.cos((+phi2+phi2-omega)/2)
	                      - k4 * sp.cos((-phi2+phi2+omega)/2)
	                    )
	d2 =  + m1*k1*sp.sin((+phi2+phi2+omega)/2) - m2*k2*sp.sin((+phi2-phi2+omega)/2) \
	     - m3*k3*sp.sin((+phi2+phi2-omega)/2) - m4*k4*sp.sin((-phi2+phi2+omega)/2)
	d2 = d2 / sp.sin(theta2/2)
	
	set_to_zero = sp.Symbol('set_to_zero')
	
	set_to_zero = sp.sin(theta2)*d2/sp.Abs(d2) + sp.sin(theta1)*d1/sp.Abs(d1)
	
	#sp.pprint(set_to_zero)
	#sp.pprint(sp.simplify(set_to_zero))
	sp.pprint(sp.trigsimp(set_to_zero))
	...
	for omega in [180.0,0.0]:
		...
		for amin,amax,astep in anglerange:
			...
			if amin == -180 and amax == 180:
				...
				if astep == 5:
					# Then we should also write (lower resolution) graphs for d and theta
					if omega == 180.0:
					...
					locallib.make2Dfigure(newX,newY,newZrca,
							fn=[output_fn+'_rca.eps',output_fn+'_rca.png'],
